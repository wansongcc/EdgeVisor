...
bool resolvePpMigrationLayers(
    ...
    // NEXT_BARRIER dynamic PP commands reuse triggerLayer as an exact
    // firstLayer on the wire. EXACT keeps its legacy trigger semantics and
    // derives the migration range from the static stage boundary.
    NnUint firstLayer = command.mode == PLAN_CMD_MODE_NEXT_BARRIER
...
void RootLlmInference::maybeApplyShiftedPpStartForStageMove(
    const std::vector<NnUint> &switchLayers,
    NnUint sourceNodeIndex,
    NnUint targetNodeIndex,
    bool selfIsSource,
    bool selfIsTarget) {
    if (executor == nullptr || switchLayers.empty()) return;
    ...
    } else if (flag == LLM_LAYER_SWITCH_RESTORE_PP_START && selfIsTarget &&
            shiftedStartLayer < toStage->endLayer) {
        executor->setShiftedPpStartLayerEnabled(shiftedStartLayer, false);
    }
}
...
bool RootLlmInference::sendPendingLayerSwitchControlOnly() {
    ...
    NnUint bypassGeneration = 0u;
    {
        ...
        if (executor != nullptr) {
            ...
            maybeApplyShiftedPpStartForStageMove(
                switchLayers,
    ...
    if (switchFromStage != nullptr && switchToStage != nullptr && !switchLayers.empty()) {
        ppStartBoundaryLayer = *std::max_element(switchLayers.begin(), switchLayers.end());
        ppStartFlag = ppStartLayerSwitchFlag(
            runtimePlan,
    ...
    if (stageBypass) {
        ...
        const NnStageConfig *previousStage = findStageByIndexLocal(plan, pendingBypassPreviousStage);
        pendingBypassPreviousNode = previousStage != nullptr ? previousStage->rootNodeIndex : 0xFFFFFFFFu;
...
void RootLlmInference::consumeStageBypassAckFrame(NnUint socketIndex, const std::vector<char> &payload) {
    ...
    std::printf("🔁 [stage-bypass-ack] cached socket=%u node=%u generation=%u stage=%u role=0x%x range=[%u,%u) count=%u\n",
        (unsigned)socketIndex, (unsigned)ack.fromNodeIndex, (unsigned)ack.bypassGeneration,
        (unsigned)ack.stageIndex, (unsigned)ack.roleFlags, (unsigned)ack.startLayer,
        (unsigned)ack.endLayer, (unsigned)ack.layerCount);
    (void)socketIndex;
...
void RootLlmInference::pollStageBypassAckFrames() {
    ...
    for (NnUint node : stageBypassExpectedAckNodes) {
        ...
        pumpWorkerFrames((NnUint)socket);
    }
}
...
void RootLlmInference::pumpWorkerFrames(NnUint socketIndex) {
    ...
    for (;;) {
        ...
        if (frame.kind == LLM_WORKER_FRAME_STAGE_BYPASS_ACK) {
            ...
            LlmPerfPacket packet{}; std::memcpy(&packet, payload.data(), sizeof(packet));
            workerProfileFrameCache[socketIndex].push_back(packet);
        } else if (frame.kind == LLM_WORKER_FRAME_SAMPLED_TOKEN && payload.size() == sizeof(LlmSampledTokenPacket)) {
    ...
    if (network != nullptr && network->nSockets > 0) {
        ...
        for (NnUint i = 0; i < nWorkers; ++i) {
            ...
            if (!workerProfileFrameCache[i].empty()) {
                ...
                workerProfileFrameCache[i].pop_front();
                continue;
            }
            LlmWorkerFrameHeader frame{};
            std::vector<char> payload;
            ...
            try {
                ...
                while (inference.consumeLayerSwitch(switchPkt)) {
                    ...
                    if (planPtr != nullptr) {
                        ...
                        if (localIsTargetStage && fromStage != nullptr && toStage != nullptr &&
                                (switchPkt.reserved0 & LLM_LAYER_SWITCH_RESTORE_PP_START) != 0u) {
                            const NnUint shiftedStartLayer = switchPkt.boundaryLayer + 1u;
                            if (shiftedStartLayer < toStage->endLayer) {
                                executor.setShiftedPpStartLayerEnabled(shiftedStartLayer, false);
                                std::printf("🔁 [worker-switch] node=%u restore pp-start layer=%u (target-stage)\n",
                                    (unsigned)nodeConfig.nodeIndex,
                                    (unsigned)shiftedStartLayer);
                                std::fflush(stdout);
                            }
                ...
                if (sawStageBypass && network != nullptr && planPtr != nullptr) {
                    ...
                    std::printf("🔁 [stage-bypass-ack] sent node=%u generation=%u stage=%u role=0x%x range=[%u,%u) count=%u chain=",
                        (unsigned)ack.fromNodeIndex, (unsigned)ack.bypassGeneration, (unsigned)ack.stageIndex,
                        (unsigned)ack.roleFlags, (unsigned)ack.startLayer, (unsigned)ack.endLayer, (unsigned)ack.layerCount);
                    for (size_t ci = 0u; ci < activeChain.size(); ++ci) std::printf("%s%u", ci == 0u ? "" : ",", (unsigned)activeChain[ci]);
                    std::printf("\n"); std::fflush(stdout);
                }
