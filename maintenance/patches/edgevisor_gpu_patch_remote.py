...
def insert_before_once(path: Path, needle: str, text: str) -> None:
    ...
    if text.strip() in s:
        return
    if needle not in s:
        raise RuntimeError(f"needle not found in {path}: {needle[:80]!r}")
    path.write_text(s.replace(needle, text + needle, 1))


# 1) Generic executor segment hooks.
p = root / "src/nn/nn-executor.hpp"
replace_once(
    p,
...
# 2) CPU segment keeps existing behavior through generic hooks.
...
# 3) Executor calls generic hooks, no longer CPU-only.
...
# 4) Vulkan header gains plan state and refresh hooks.
...
# 5) App passes initial partition plan into Vulkan device.
p = root / "src/app.cpp"
replace_once(
    p,
...
print("gpu interface patch applied")
