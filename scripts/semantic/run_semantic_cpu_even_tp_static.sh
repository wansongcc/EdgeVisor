...
LOG_DIR="${EDGEVISOR_LOG_ROOT}/semantic/cpu_even_tp_static_2plus2_${STAMP}"
mkdir -p "${LOG_DIR}"
cd "${BASE}" || exit 2
...
P1=$!
./dllama worker --port "${PORT2}" --nthreads 2 >"${LOG_DIR}/worker2.log" 2>&1 &
P2=$!
sleep 4
...
./dllama inference \
  ...
  --temperature 0 \
  --seed 1 \
  --workers "127.0.0.1:${PORT1}" "127.0.0.1:${PORT2}" >"${LOG_DIR}/root.log" 2>&1
RC=$?
