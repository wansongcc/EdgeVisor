...
LOG_DIR="${EDGEVISOR_LOG_ROOT}/semantic/gpu_dynamic_heads_${STAMP}"
mkdir -p "${LOG_DIR}"
cd "${BASE}" || exit 2
PORT1=19701
PORT2=19702
SOCK=/tmp/dllama_semantic_gpu_heads.sock
...
grep -E "plan|apply|emit|Pred|EOS|Critical|error" "${LOG_DIR}/root_gpu0.log" | tail -n 180 || true
echo "--- generated text ---"
grep -E "Pred|EOS" "${LOG_DIR}/root_gpu0.log" || true
exit "${RC}"
